...
-- Quick tasks: standalone tasks that can be assigned to arbitrary students
...
CREATE TABLE quick_task_assignments (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE INDEX idx_quick_tasks_created_at ON quick_tasks(created_at);
CREATE INDEX idx_quick_task_assignments_student ON quick_task_assignments(student_id);

-- Quick tasks policies: admins/staff can manage, students can view assigned quick tasks
